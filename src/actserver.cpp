...
void odomCallback(const nav_msgs::Odometry::ConstPtr& msg) {
    odomFeedback.base_position.pose = msg->pose.pose;
}
...
move_base_msgs::MoveBaseFeedback *odomFeedback,move_base_msgs::MoveBaseResult *goalResult) 
{
    ...
    {
    currentEcludiandistance=sqrt(pow(odomFeedback->base_position.pose.position.x, 2) + pow(odomFeedback->base_position.pose.position.y, 2));
    ...
    else if (processing_time.toSec() > 30.0) {
